...
static __global__ void moe_vec_q(
  ...
  /* FT-EXP-STRIDE: byte stride from the tensor, not nrows * row_bytes -- so a bank
     region may be wider than the rows this layer actually stores. */
  const block_q_t* x =
      (const block_q_t*)((const char*)vx + (int64_t)expert * exp_stride);
...
static void moe_vec_q4_0_q8_1_cuda(
  ...
  moe_vec_q<scalar_t, QK4_0, QI4_0, block_q4_0, VDR_Q4_0_Q8_1_MMVQ, vec_dot_q4_0_q8_1>
      <<<block_nums, block_dims, 0, stream>>>(vx, vy, dst, topk_ids, top_k, ncols, nrows, token_stride, exp_stride);
}
